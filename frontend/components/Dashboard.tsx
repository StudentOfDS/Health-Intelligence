...
export default function Dashboard() {
  ...
  async function createUser() {
    setError(null);
    try {
      const created = await postJson<User>("/users", { email, password });
      setUser(created);
      const auth = await postJson<{ access_token: string }>("/auth/login", { email, password });
  ...
  async function saveBaseline() {
    ...
    setError(null);
    try {
      await postJson("/baseline", { user_id: user.id, ...baseline }, token ?? undefined);
    } catch (e) {
  ...
  async function saveDailyLog() {
    ...
    setError(null);
    try {
      await postJson("/logs", { user_id: user.id, ...daily }, token ?? undefined);
    } catch (e) {
  ...
  async function runAnalysis() {
    ...
    setError(null);
    try {
      const result = await getJson<Record<string, unknown>>(`/users/${user.id}/analysis`, token ?? undefined);
      setAnalysis(result);
    } catch (e) {
  ...
  return (
    <main>
      ...
      <section>
        ...
        <button onClick={createUser}>Create User + Login</button>
